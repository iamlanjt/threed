...
function checkAllIntersections(origin, components, r) {
    ...
    components.forEach((c)=>{
        c.vertexes.forEach((triangle)=>{
            ...
            let intersection = r.intersectsTriangle([tri1, tri2, tri3])
...
export class Stage {
    ...
    render(camera, zeros_stream, options) {
        ...
        let viewDepth = options.viewDepth || 99999
        let resX = resolution[0]
        let resY = resolution[1]
        for (let IY = 0; IY < resY; IY++) {
            for (let IX = 0; IX < resX; IX++) {
                let originX = (IX-resolution[0])/resolution[0]
                let originY = (IY-resolution[1])/resolution[1]
                let thisDir = new Point3(originX, originY, viewDepth)
                const thisRay = new Ray(camera.position.array, thisDir)
